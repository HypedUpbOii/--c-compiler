...
void Lexer::write_token(const std::string &type) {
    outstream << "\tToken Name: " << type << "\tLexeme: " << YYText()
              << "\tLineno: " << lineno() << std::endl;
    next_token = type;
}

void Lexer::lexerror() {
    std::cerr << "sclp error: File: " << input_file << ", Line: " << lineno()
              << ", Next token: " << next_token << ", Lexeme: \"" << YYText()
...
void parser::Parser::error(const location_type &loc, const std::string &msg) {
    ...
    std::cerr << "sclp error: File: " << lexer.input_file
              << ", Line: " << lexer.lineno()
              << ", Next token: " << lexer.next_token << ", Lexeme: \""
              << lexer.YYText()
